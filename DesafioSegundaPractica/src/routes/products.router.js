...
export default class ProductRouter extends BaseRouter {
    init() {
        ...
        //CATEGORY FILTER
        this.get('/:category', ['PUBLIC'], passportCall('jwt', { strategyType: 'jwt', session: false }), async (req, res) => {
            try {
                const { page = 1 } = req.query;
                const { limit = 10 } = req.query;
                const { sort } = req.query;
        ...
        //GET BY ID
        this.get('/:pid', ['PUBLIC'], passportCall('jwt', { strategyType: 'jwt', session: false }), async (req, res) => {
            try {
        ...
        //POST
        this.post('/', ['PUBLIC'], passportCall('jwt', { strategyType: 'jwt', session: false }), async (req, res) => {
            try {
        ...
        // //PUT
        this.put('/:pid', ['PUBLIC'], passportCall('jwt', { strategyType: 'jwt', session: false }), async (req, res) => {
            try {
        ...
        //DELETE
        this.delete('/:pid', ['PUBLIC'], passportCall('jwt', { strategyType: 'jwt', session: false }), async (req, res) => {
            try {
                const { pid } = req.params;
